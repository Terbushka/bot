...
class TelegramUploader:
    ...
    async def _msg_to_reply(self):
        if self._listener.up_dest:
            # Зберігаємо цільовий чат без відправки попереднього текстового повідомлення
            self._up_chat_id = self._listener.up_dest
            self._up_thread_id = self._listener.chat_thread_id
            try:
                if self._user_session:
                    chat = await TgClient.user.get_chat(self._listener.up_dest)
                else:
                    chat = await self._listener.client.get_chat(self._listener.up_dest)
            ...
            self._up_chat_id = self._sent_msg.chat.id
            self._up_thread_id = getattr(self._sent_msg, "message_thread_id", None)
        else:
    ...
    async def _upload_file(self, cap_mono, file, o_path, force_document=False):
        ...
        try:
            ...
            if (
                ...
                if thumb == "none":
                    thumb = None
                self._sent_msg = await self._send_file_direct(
                    "send_document",
